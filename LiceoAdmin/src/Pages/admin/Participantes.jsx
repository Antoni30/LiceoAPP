...
const fetchUsuariosDisponibles = async () => {
  try {
    ...
    for (const profesor of profesores) {
      const estaDisponible = await verificarDisponibilidadProfesor(profesor.idUsuario);
      if (estaDisponible) {
...
// Verifica si el profesor no está asignado a ningún curso
const verificarDisponibilidadProfesor = async (idUsuario) => {
  try {
    const response = await fetch(
    ...
    if (!response.ok) {
      throw new Error("Error al verificar disponibilidad del profesor");
    }

    const cursosAsignados = await response.json();
    return cursosAsignados.length === 0; // true si no tiene cursos
  } catch (err) {
    console.error(`Error verificando disponibilidad de profesor ${idUsuario}:`, err);
  ...
  const agregarProfesorAlCurso = async () => {
    ...
    try {
      ...
      // 2. Asignar al curso
      const response = await fetch("http://localhost:8080/api/usuarios-cursos", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || "Error al asignar profesor al curso");
      }
        ...
        usuariosCursos.map(async (usuarioCurso) => {
          ...
          if (rolData.length > 0) {
            ...
            if (nombreRolResponse.ok) {
              ...
              nombreRol =
                nombreRolData.nombre || `Rol ID: ${rolData[0].idRol}`;
            }
          }
        ...
        <div className="bg-red-50 border-l-4 border-red-500 p-4 mx-auto max-w-4xl mt-8">
          <div className="flex">
            ...
            <div className="ml-3">
              ...
              <h3 className="text-sm font-medium text-red-800">Error al cargar participantes</h3>
          ...
          <div className="bg-white shadow-md rounded-lg overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-indigo-50 to-blue-50">
              ...
              <div className="flex justify-between items-center">
                <div>
                  <h1 className="text-2xl font-semibold text-gray-800">Participantes del Curso</h1>
                  <p className="mt-1 text-sm text-gray-600">Lista de todos los participantes inscritos en este curso</p>
                </div>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {participantes.length > 0 ? (
                    participantes.map((participante, index) => (
                      <tr key={participante.idUsuario} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50 hover:bg-gray-50'}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full 
                            ${participante.rol.toLowerCase().includes('estudiante') ? 'bg-green-100 text-green-800' :
                              participante.rol.toLowerCase().includes('profesor') ? 'bg-blue-100 text-blue-800' :
                              participante.rol.toLowerCase().includes('admin') ? 'bg-purple-100 text-purple-800' :
                    ...
                    <tr>
                      <td colSpan="5" className="px-6 py-4 text-center text-sm text-gray-500">
                        <div className="flex flex-col items-center justify-center py-8">
                          ...
                          <h3 className="mt-2 text-sm font-medium text-gray-700">No hay participantes</h3>
                          <p className="mt-1 text-sm text-gray-500">No se encontraron participantes inscritos en este curso.</p>
                          <button
            ...
            {participantes.length > 0 && (
              <div className="px-6 py-3 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
                <div className="text-sm text-gray-500">
                  Mostrando <span className="font-medium">{participantes.length}</span> participantes
                </div>
              </div>
            )}
          </div>
        ...
        <div className="fixed inset-0 bg-amber-50 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-medium text-gray-900">Agregar Profesor al Curso</h3>
              <button
                onClick={() => {
                  setShowAddProfessor(false);
                  setAddError(null);
